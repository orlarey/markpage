...
import {
  LocalDocServer,
  markpageUrlFor,
} from "../vscode/src/local-server-core";
...
const get = (url: string, origin = APP) =>
...
describe("LocalDocServer", () => {
  ...
  it("serves the editor buffer (unsaved edits) for an open document", async () => {
    ...
    expect(await (await get(await server.share(file))).text()).toBe(
  ...
  it("refuses anything outside the shared folder, or without the token", async () => {
    ...
    expect((await get(new URL("../secret/keys.txt", url).href)).status).toBe(
      404
    );
    ...
    expect((await get(`${base.origin}/${token}${other}/keys.txt`)).status).toBe(
      404
    );
    ...
    expect(
      (
        await get(
          `${base.origin}/nottoken${base.pathname.slice(token.length + 1)}`
        )
      ).status
    ).toBe(404);
  });
  ...
  it("lets only the markpage origin read, and answers the private-network preflight", async () => {
    ...
    expect(
      (await fetch(url, { method: "POST", headers: { Origin: APP } })).status
    ).toBe(405);
  });
  ...
  it("writes the shared document back over the version markpage read", async () => {
    ...
    expect(
      (await put("# Modifié", { "X-Markpage-Base": sha("# Sur disque") }))
        .status
    ).toBe(204);
    ...
    expect((await put("# Écrase", { "X-Markpage-Force": "1" })).status).toBe(
      204
    );
  ...
  it("writes nothing but the shared documents, for no one but markpage", async () => {
    ...
    expect(
      (await fetch(url, { method: "PUT", body: "x", headers: force })).status
    ).toBe(403);
    expect(
      (
        await fetch(url, {
