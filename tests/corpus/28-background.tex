...
\title{Page backdrops — \textbackslash{}texttt\{::: background\}}
...
A logo dropped into the top-right corner (\texttt{at=x,y} in [0,1]², \texttt{size} =
...
A centred title block over a dark cover — a \texttt{::: style} nested inside a
\texttt{::: background} (four colons on the outer fence so it closes correctly):
