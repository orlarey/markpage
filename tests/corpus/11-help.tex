...
\section{Encore d'autres éléments Markdown}
...
\subsection{Code en ligne et blocs de code}
...
\section{Gérer plusieurs documents}
...
\subsection{Exporter votre document}
...
\section{Personnaliser le rendu PDF (Réglages)}
...
de vos choix — la cohérence de la zone de saisie ne change pas.}
  \item \textbf{Tailles et couleurs} des titres (h1 à h4) et du corps
  \item \textbf{Style du code} (taille et couleur)
  \item \textbf{Style de la citation} (taille, couleur du texte, couleur de la
barre verticale)
  \item \textbf{Numéro de page} : position, taille, couleur, italique
\end{itemize}


Les réglages sont \textbf{mémorisés entre vos sessions}. Le bouton
\emph{Réinitialiser} en bas du panneau revient aux valeurs par défaut.
...
\section{Tableaux de données (CSV / TSV)}
...
\section{Encadrés (notes, théorèmes…)}
...
\subsection{Format CSV : virgules françaises}
...
\subsection{Séries chronologiques}
