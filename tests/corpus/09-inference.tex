...
A rule with two premises on the same line (separated by \texttt{;}) and a label:


\[
\dfrac{\Gamma \vdash f : A \to B \quad \Gamma \vdash x : A}{\Gamma \vdash f\,x : B} \quad \textsf{(T-App)}
\]


A rule using Unicode operators (the inference body goes to LaTeX as-is and relies on \texttt{newunicodechar}/the math mode for rendering):


\[
\dfrac{}{\Gamma, x : \tau \vdash x : \tau} \quad \textsf{(Ax)}
\]
