...
\title{Footnotes and definition lists}
...
A paragraph with a footnote reference\footnote{First footnote with \textbf{bold} and a \href{https://example.com}{link}.} and a second one\footnote{Second footnote, a different one.}. The first reference inlines the definition into \texttt{\textbackslash{}footnote\{\}}; subsequent references to the same id use \texttt{\textbackslash{}footnotemark[N]}.


Re-using the first footnote\footnotemark[1] should not duplicate the body.
...
\begin{description}
  \item[Coffee] A dark drink brewed from roasted beans.
  \item[Coffee] Also a common reason engineers stay awake.
\end{description}

\begin{description}
  \item[Tea] An infusion of leaves in hot water.
\end{description}
\end{document}
