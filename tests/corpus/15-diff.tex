...
A pre-\texttt{feat}/\texttt{fix} snippet, followed by the diff that lands the
change. The dispatcher classifies each line by its leading marker:
\texttt{+} / \texttt{-} for additions / removals, \texttt{@@} for hunks, \texttt{+++} / \texttt{---}
...
Inline code like \texttt{git diff} flows through unchanged because only
