...
- **Lier à GitHub** — l'utilisateur saisit `owner/repo`, branche, chemin. Si le
  ...
  *prendre la version distante*) ; sinon écrire le bundle et mémoriser le
  `baselineSha`.
- **Save (commit & push)** — après le commit local : pour `content.md` et chaque
  `assets/<sha>.<ext>`, `PUT contents` avec le `sha` connu. Le message de commit
  par défaut : `markpage: <nom du document>`. Met à jour `baselineSha`.
...
sequenceDiagram
    ...
    L->>G: PUT content.md + assets/<sha>.png
...
- **Menu Fichier** (gated : token présent) :
  - *Ouvrir depuis GitHub…* — saisie `owner/repo` + navigateur de chemins.
...
- **Granularité des commits** : un commit par Save (simple) vs. regrouper
  `content.md` + assets en un seul commit (API *git trees*, plus complexe) ?
- **Découverte des documents** : lister les bundles d'un dépôt (dossiers
  contenant un `content.md`) pour un *Ouvrir depuis GitHub* convivial ?
- **Message de commit** : fixe (`markpage: <nom>`) ou éditable par l'utilisateur ?
- **Multi-fichiers / assets supprimés** : nettoyer les `assets/` orphelins côté
  dépôt lors d'un Save ?
...
5. **Ouvrir depuis GitHub** — saisie cible + (option) navigateur de chemins.
